<?php
/**
* Ngo Charity Custom Theme Settings
*
* @package NgoCharity
*/

define('NGO_SHORTNAME', 'ngo'); // used to prefix the individual setting field id see ngo_options_page_fields()
define('NGO_PAGE_BASENAME', 'ngo-settings'); // the settings page slug
 ...
 */
add_action( 'admin_menu', 'ngo_add_menu' );
add_action( 'admin_init', 'ngo_register_settings' );

/* 
 * Group scripts (js & css)
 */
function ngo_settings_scripts(){
    // wp_enqueue_style('ngo_theme_settings_css', get_template_directory_uri() . '/lib/css/ngo_theme_settings.css');
    wp_enqueue_script( 'ngo_theme_settings_js', get_template_directory_uri() . '/inc/admin-panel1/themes.js', array('jquery'));
}

/*
 * The Admin menu page
 */
function ngo_add_menu(){
     
    // Display Settings Page link under the "Appearance" Admin Menu
    // add_theme_page( $page_title, $menu_title, $capability, $menu_slug, $function);
    $ngo_settings_page = add_theme_page(__('Theme Options', 'ngoCharity'), __('Theme Options','ngoCharity'), 'manage_options', NGO_PAGE_BASENAME, 'ngo_settings_page_fn');         

    // css & js
    add_action( 'load-'. $ngo_settings_page, 'ngo_settings_scripts' );
}
 ...
 * @return array
 */
function ngo_get_settings() {
     
    $output = array();
     
    // put together the output array
    $output['ngo_option_name']       = 'ngo_options'; // the option name as used in the get_option() call.
    $output['ngo_page_title']        = __( 'Theme Settings Page','ngo_textdomain'); // the settings page title
    $output['ngo_page_sections']     = ngo_options_page_sections(); // the setting section
    $output['ngo_page_fields']       = ngo_options_page_fields(); // the setting fields
    $output['ngo_contextual_help']   = ''; // the contextual help
 ...
 */
function ngo_create_settings_field( $args = array() ) {
    ...
    $defaults = array(
        ...
        'section' => 'main_section',                     // the section this setting belongs to — must match the array key of a section in ngo_options_page_sections()
    ...
    // additional arguments for use in form field output in the function ngo_form_field_fn!
    $field_args = array(
        'type'      => $type,
        'id'        => $id,
        'desc'      => $desc,
    ...
    add_settings_field( $id, $title, 'ngo_form_field_fn', __FILE__, $section, $field_args );
...
function ngo_register_settings(){
    ...
    $settings_output = ngo_get_settings();
    $ngo_option_name = $settings_output['ngo_option_name'];
     
    //setting
    //register_setting( $option_group, $option_name, $sanitize_callback );
    register_setting($ngo_option_name, $ngo_option_name, 'ngo_validate_options' );

    //sections
    // add_settings_section( $id, $title, $callback, $page );
    if(!empty($settings_output['ngo_page_sections'])){
        // call the "add_settings_section" for each!
        foreach ( $settings_output['ngo_page_sections'] as $id => $title ) {
            add_settings_section( $id, $title, 'ngo_section_fn', __FILE__);
        }
    }

    //fields
    if(!empty($settings_output['ngo_page_fields'])){
        // call the "add_settings_field" for each!
        foreach ($settings_output['ngo_page_fields'] as $option) {
            ngo_create_settings_field($option);
        }
    }
}
 ...
 */
function ngo_settings_page_fn() {
// get the settings sections array
    $settings_output = ngo_get_settings();
?>
    <div class="wrap">
        <div class="icon32" id="icon-options-general"></div>
        <h2><?php echo $settings_output['ngo_page_title']; ?></h2>
         
        <form action="options.php" method="post">
            <?php
            // http://codex.wordpress.org/Function_Reference/settings_fields
            settings_fields($settings_output['ngo_option_name']);
             
            // http://codex.wordpress.org/Function_Reference/do_settings_sections
            do_settings_sections(__FILE__);
            ?>
            <p class="submit">
                <input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Save Changes','ngo_textdomain'); ?>" />
            </p>
 ...
 * @return echoes output
 */
function  ngo_section_fn($desc) {
    echo "<p>" . __('Settings for this section','ngo_textdomain') . "</p>";
}
 ...
 * @return echoes output
 */
function ngo_form_field_fn($args = array()) {
    ...
    $ngo_option_name = $settings_output['ngo_option_name'];
    $options            = get_option($ngo_option_name);
    ...
    switch ( $type ) {
        case 'text':
            ...
            echo "<input class='regular-text$field_class' type='text' id='$id' name='" . $ngo_option_name . "[$id]' value='$options[$id]' />";
        ...
        case "multi-text":
            foreach($choices as $item) {
                ...
                $item[0] = esc_html__($item[0], 'ngo_textdomain');
                if (!empty($options[$id])) {
                ...
                echo "<span>$item[0]:</span> <input class='$field_class' type='text' id='$id|$item[1]' name='" . $ngo_option_name . "[$id|$item[1]]' value='$value' /><br/>";
            }
        ...
        case 'textarea':
            ...
            echo "<textarea class='textarea$field_class' type='text' id='$id' name='" . $ngo_option_name . "[$id]' rows='5' cols='30'>$options[$id]</textarea>";
            echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";        
        break;
         
        case 'select':
            echo "<select id='$id' class='select$field_class' name='" . $ngo_option_name . "[$id]'>";
                foreach($choices as $item) {
                    $value  = esc_attr($item, 'ngo_textdomain');
                    $item   = esc_html($item, 'ngo_textdomain');
        ...
        case 'select2':
            ...
            foreach($choices as $item) {
                ...
                $item = explode("|",$item);
                $item[0] = esc_html($item[0], 'ngo_textdomain');
        ...
        case 'checkbox':
            echo "<input class='checkbox$field_class' type='checkbox' id='$id' name='" . $ngo_option_name . "[$id]' value='1' " . checked( $options[$id], 1, false ) . " />";
            echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
        break;
        ...
        case "multi-checkbox":
            foreach($choices as $item) {
                ...
                echo "<input class='checkbox$field_class' type='checkbox' id='$id|$item[1]' name='" . $ngo_option_name . "[$id|$item[1]]' value='1' $checked /> $item[0] <br/>";
            }
            echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
        break;
    }
 ...
 * @return array
 */
function ngo_validate_options($input) {
    ...
    // collect only the values we expect and fill the new $valid_input array i.e. whitelist our option IDs
        ...
        $options = $settings_output['ngo_page_fields'];
                    ...
                    switch ( $option['class'] ) {
                        ...
                        case 'numeric':
                            ...
                            if(is_numeric($input[$option['id']]) == FALSE) {
                                add_settings_error(
                                    ...
                                    NGO_SHORTNAME . '_txt_numeric_error', // error ID
                                    __('Expecting a Numeric value! Please fix.','ngo_textdomain'), // error message
                        ...
                        case 'multinumeric':
                            ...
                            if($input[$option['id']] !=''){
                                ...
                                $valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : __('Expecting comma separated numeric values','ngo_textdomain');
                            }else{
                            ...
                            if($input[$option['id']] !='' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
                                add_settings_error(
                                    $option['id'], // setting title
                                    NGO_SHORTNAME . '_txt_multinumeric_error', // error ID
                                    __('Expecting comma separated numeric values! Please fix.','ngo_textdomain'), // error message
                                    'error' // type of message
                                );
                            }
                        break;
                        ...
                        case 'email':
                            ...
                            $input[$option['id']]       = trim($input[$option['id']]); // trim whitespace
                            if($input[$option['id']] != ''){
                                $valid_input[$option['id']] = (is_email($input[$option['id']])!== FALSE) ? $input[$option['id']] : __('Invalid email! Please re-enter!','ngo_textdomain');
                            }elseif($input[$option['id']] == ''){
                                $valid_input[$option['id']] = __('This setting field cannot be empty! Please enter a valid email address.','ngo_textdomain');
                            }
                            ...
                            if(is_email($input[$option['id']])== FALSE || $input[$option['id']] == '') {
                                add_settings_error(
                                    $option['id'], // setting title
                                    NGO_SHORTNAME . '_txt_email_error', // error ID
                                    __('Please enter a valid email address.','ngo_textdomain'), // error message
                                    'error' // type of message
                                );
                            }
                        break;
 ...
 */
function ngo_show_msg($message, $msgclass = 'info') {
 ...
 *
 * @return calls ngo_show_msg()
 */
function ngo_admin_msgs() {
     
    // check for our settings page - need this in conditional further down
    $ngo_settings_pg = strpos($_GET['page'], ngo_PAGE_BASENAME);
    ...
    if(current_user_can ('manage_options') && $ngo_settings_pg !== FALSE && !empty($set_errors)){
 
        // have our settings succesfully been updated?
        if($set_errors[0]['code'] == 'settings_updated' && isset($_GET['settings-updated'])){
            ngo_show_msg("<p>" . $set_errors[0]['message'] . "</p>", 'updated');
        ...
        // have errors been found?
            ...
            foreach($set_errors as $set_error){
                ...
                ngo_show_msg("<p class='setting-error-message' title='" . $set_error['setting'] . "'>" . $set_error['message'] . "</p>", 'error');
            }
        }
    }
}
...
// admin messages hook!
add_action('admin_notices', 'ngo_admin_msgs');
